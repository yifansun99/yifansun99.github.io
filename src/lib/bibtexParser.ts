...
/**
 * Parse the small, trusted BibTeX data file used by this static site.
 * Keeping the parser local avoids shipping the abandoned bibtex-parse-js
 * dependency and its legacy Babel/test dependency tree.
 */
function parseBibTeXEntries(input: string): RawBibTeXEntry[] {
  ...
  while (cursor < input.length) {
    ...
    const entryType = input.slice(at + 1, open).trim();
    let depth = 1;
    let inQuote = false;
    let close = open + 1;

    for (; close < input.length && depth > 0; close += 1) {
      const char = input[close];
      const escaped = input[close - 1] === '\\';
    ...
    if (depth !== 0) {
      throw new Error(`Unbalanced BibTeX entry beginning at character ${at}`);
    }

    const body = input.slice(open + 1, close - 1);
    const comma = findTopLevelDelimiter(body, ',');
    if (comma === -1) {
      cursor = close;
      continue;
    }
    ...
    const fieldSource = body.slice(comma + 1);
    const entryTags: Record<string, string> = {};

    for (const field of splitTopLevel(fieldSource, ',')) {
      const equals = findTopLevelDelimiter(field, '=');
      if (equals === -1) continue;
      const key = field.slice(0, equals).trim().toLowerCase();
      const value = unwrapBibTeXValue(field.slice(equals + 1).trim());
      if (key) entryTags[key] = value;
    }
...
function findTopLevelDelimiter(input: string, delimiter: string): number {
  let depth = 0;
  let inQuote = false;
  ...
  for (let index = 0; index < input.length; index += 1) {
    ...
    if (!inQuote) {
      if (char === '{') depth += 1;
      if (char === '}') depth -= 1;
      if (char === delimiter && depth === 0) return index;
    }
  }
